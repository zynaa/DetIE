...
@pytest.fixture
def texts_and_patterns() -> Tuple[List[str], List[List[str]]]:
    return [
...
def test_find_spans(texts_and_patterns: pytest.fixture):
    ...
    for i, (text, pattern) in enumerate(zip(*texts_and_patterns)):
        all_spans = find_spans(text, pattern)

        for spans in all_spans:
            for span in spans:
                assert text[span[0] : span[1]] in pattern[0]

        assert len(filter_tokens(pattern[0])) == len(filter_tokens("".join(
            [text[span[0] : span[1]] for spans in all_spans for span in spans])))

        if 1 <= i <= 3:
            for spans in all_spans:
                assert len(spans) == 1
                span = spans[0]
                assert filter_tokens(text[span[0] : span[1]]) == filter_tokens(pattern[0])
